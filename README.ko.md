...
1. <kbd>SHIFT (⇧)</kbd> + <kbd>OPTION (⌥)</kbd> + <kbd>D</kbd>를 눌러 Kiwa 패널을 열거나, 메뉴바의 `"` 아이콘을 클릭합니다.
2. <kbd>⌘1</kbd>~<kbd>⌘9</kbd> 키로 기호를 선택하거나, 화살표 키와 <kbd>Return</kbd>으로 선택합니다.
3. 기호가 클립보드에 복사되고 효과음과 함께 패널이 자동으로 닫힙니다.
4. <kbd>⌘V</kbd>로 원하는 곳에 붙여넣습니다.
5. 패널을 닫으려면 <kbd>ESC</kbd>를 누르거나 바깥 아무 곳이나 클릭합니다.
6. 메뉴바 아이콘을 우클릭하면 관하여, 종료에 바로 접근할 수 있습니다.
7. 동작을 커스터마이즈하려면 <kbd>COMMAND (⌘)</kbd> + <kbd>,</kbd>로 설정을 엽니다.
